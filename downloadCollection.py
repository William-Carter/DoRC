...
def main():
    ...
    argParser.add_argument(
        "-x", required=True, help="Your credentials file. See the readme for help on formatting it correctly.")
    argParser.add_argument("-s", required=True,
                           help="The subreddit your collection belongs to.")
    argParser.add_argument(
        "-c", required=True, help="The ID of your collection. This is the end part of the URL (Without the slashes).")
    argParser.add_argument(
        "-d", help="The directory to download files to. Will be downloaded in the same folder as the script by default.")
    ...
    elif not (credentials["password"] and credentials["username"] and credentials["client_id"] and credentials["client_secret"]):
        raise Exception("Credential file is incomplete!")

    downloadCollection(credentials["client_id"], credentials["client_secret"],
                       credentials["username"], credentials["password"], args.s, args.c, args.d)
...
def downloadCollection(clientID, clientSecret, username, password, subreddit, collectionID, dlDirectory=""):
    ...
    for link in collection.sorted_links:
        print(list(collection.sorted_links).index(
            link)+1, "of", len(collection.sorted_links))
